...
void USART3_IRQHandler(void)  //串口中断
{
  ...
  if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)
  {
    ...
    rec_data = USART_ReceiveData(USART3);
		//避免串口干扰
		if(rec_data == COMM_STOP || rec_data == COMM_UP || rec_data == COMM_DOWN || rec_data == COMM_LEFT || rec_data == COMM_RIGHT|| rec_data == COMM_AUTO|| rec_data == COMM_ESC|| rec_data == COMM_7|| rec_data == COMM_8|| rec_data == COMM_9)
		{
			if(rec_data == COMM_STOP || rec_data == COMM_ESC) {b=0;}
			else if(rec_data == COMM_AUTO) {b=1;}
			bt_rec_flag = 1;
			ctrl_comm = rec_data;
			continue_time = 40;		
		}
